    ...
    <div class="reg-form d-flex align-items-center justify-content-center mt-5">
        <form class="registration-form" method="POST" action="{{ route('login') }}">
            ...
            <div class="inputForm">
                ...
                <input placeholder="Enter your Password" class="input" type="password" name="password" required autocomplete="current-password" :value="__('Password')">
            </div>
            @error('password')
            <p class="error text-danger mb-0">{{ $message }}</p>
            @enderror
            ...
            <p class="p">don't have an account? <a class="span" href="{{ route('register') }}">Register Now</a></p>
        </form>
